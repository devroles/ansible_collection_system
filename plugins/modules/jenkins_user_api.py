...
def main():
    ...
    module = AnsibleModule(
        argument_spec={
            ...
            "cli_jar": {"default": "/var/cache/jenkins/war/WEB-INF/jenkins-cli.jar"},
    ...
    groovy.write(six.ensure_binary(MYFILE.format(args.user)))
    groovy.close()
    process = [
        args.java_command,
        "-jar",
    ...
    try:
        ...
        if p.returncode == 0:
            module.exit_json(
                api_key=output.strip(), err=err, changed=False, success=True
            )
        else:
    ...
    module.fail_json(msg=msg, stderr=err, stdout=output, api_key="")
