...
createOrUpdateUser("{{ jenkins_configure_admin.name }}",
                   "{{ jenkins_configure_admin.email }}",
                   "{{ jenkins_configure_admin.password }}");
{% for user in jenkins_configure_local_users %}
createOrUpdateUser("{{ user.name }}", "{{ user.email }}", "{{ user.password }}");
{% endfor %}
